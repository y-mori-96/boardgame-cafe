            ...
            <div class="p-6 bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="md:flex md:justify-around">
                    ...
                    <div class="md:w-1/2 ml-4 flex flex-col justify-around">
                        ...
                        <p>以下管理者のみ表示</p>
                        <div class="flex justify-between items-center">
                            <button onclick="location.href='{{ route('admin.rental-items.edit', $rental_item) }}'" class="flex ml-auto text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded">
                                編集
                            </button>
                            {{--
                            <button onclick="location.href='{{ route('admin.exhibitions.edit', $exhibition) }}'" class="flex ml-auto text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded">
                                画像編集
                            </button>
                            --}}
                            <form method="post" action="{{ route('admin.rental-items.destroy', $rental_item) }}">
                                @csrf
                                @method('delete')
                                <x-primary-button class="ml-4 bg-red-700 hover:bg-red-600 focus:bg-red-600 active:bg-red-800">
                                    削除
                                </x-primary-button>
                            </form>
                        </div>
