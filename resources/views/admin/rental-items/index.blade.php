                        ...
                        @forelse( $rental_items as $rental_item ) 
                          <div class="xl:w-1/4 md:w-1/2 p-4">
                            ...
                            　　      @else
                                    ...
                                    @endif
                                    <div class="absolute left-5 bottom-5 flex items-center justify-center bg-black bg-opacity-60 rounded p-3">
                                        <span class="text-white text-sm">{{ $rental_item->state == 1 ? '貸出可' : '貸出中' }}</span>
                                    </div>
                                    {{-- 売り切れ判定 
                                    @if($rental_items->isSold())
                                    @endif
                                    <div class="absolute bottom-0 w-full bg-black bg-opacity-60 text-white text-xs py-1 px-2 text-right rounded-b">
                                        {{ $rental_items->price }} 円
                                    </div>
                                    --}}
                                </a>
                                ...
                                <p class="leading-relaxed text-base">
                                    <font style="vertical-align: inherit;">
                                        <font style="vertical-align: inherit;">在庫数：{{ $rental_item->stock_quantity }}</font>
                                    </font>
                                </p>
                            </div>
                          </div>
